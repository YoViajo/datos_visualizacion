...
## Distancia a centros de salud en Bolivia (2016)
## Superficie hexagonal de 10 km

## Basado en:
# Ref: https://github.com/HudsonJamie/tidy_tuesday/blob/main/2021/week_46/afrilearndata.R
# Jamie Hudson
...
bolivia_hex <- readOGR(dsn = "dat/Bolivia_hex_10km.geojson")
...
bolivia_hospital <- read.csv(file = 'dat/est_salud_2016_nivel3_geom.csv')
...
ggplot() +
  ...
  labs(title = "Distancia a Centros de Salud",
       ...
       caption = "@rcrmj | fuente = {GeoBolivia 2016}") +
  theme_void() +
...
ggsave(paste0("bolivia_hospitals_", format(Sys.time(), "%d%m%Y"), ".png"),
       dpi = 320,
       width = 5,
       height = 5)
